...
function RunMainContent() {
    ...
    StopLoadingAnimation()
        ...
        function setTabVariables(elm) {  
            ...
            if (!elm) return; // stop if elm doesn't exist
        ...
        async function averageHandling(periodNumber , hasClassAverage=false){
            ...
            if (token != null && pupilId){
                if (Data != null){
                    inRequest = false
                    averageDataHandling(Data, periodNumber , hasClassAverage)
                } else {
        ...
        function ChangeOnTabClick(){
            [...document.querySelectorAll("[data-toggle='tab']")].forEach(elm => {                         
                elm.addEventListener("click", () => { 
                    ...
                    console.log(modeEvaluation,modeMoyenne,modeGraphic)
        ...
        function graphicMain(periodId){
            ChangeOnTabClick()
            GraphicModeHandler()
            ChangeOnBtnGraphicClick(periodId)
        }
        ...
        function runMainAverage(){
            waitForElm(".ng-star-inserted.active.sousperiode").then(elm => {
                ...
                setPeriodId()
                const periodID = getCurrentTabID()
                graphicMain(periodID)
                averageHandling(periodID)
